...
impl BorderShader {
	...
	pub fn element(
		...
		let thickness: f32 = CONFIG.read().window_decorations.borders.width as f32;
		...
		if let Some(elem) = elements.get_mut(window) {
			...
			let elem = if CONFIG.read().window_decorations.borders.radius > 0.0 {
				PixelShaderElement::new(
					...
					vec![
						...
						Uniform::new(
							"radius",
							CONFIG.read().window_decorations.borders.radius as f32
								+ thickness + 2.0,
						),
